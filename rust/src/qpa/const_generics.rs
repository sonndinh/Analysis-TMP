...
impl<T: TaskTrait, U: Dmin> Dmin for Tasklist<T, U> {
    const RESULT: u32 = Self::dmin_value();
}

impl Dmin for NullTask {
    const RESULT: u32 = u32::MAX;
}

impl<T: TaskTrait, U: Dmin> Tasklist<T, U> {
    const fn dmin_value() -> u32 {
        if T::DEADLINE < <U as Dmin>::RESULT {
            T::DEADLINE
        } else {
            <U as Dmin>::RESULT
        }
...
impl<T: TaskTrait, U: Dmin + MaxDelta + LastarHelper + TotalUtil> Tasklist<T, U> {
    const fn qpa_predicate<const DURATION: u64>() -> bool where U: Pdf<DURATION> {
        <Self as Pdf::<DURATION>>::RESULT <= DURATION && <Self as Pdf::<DURATION>>::RESULT > Self::dmin_value() as u64
    }

    const LASTAR: u64 = <Self as Lastar>::RESULT;
    const INIT_DURATION: u64 = <Self as Dmax<LASTAR>>::RESULT;
